...
def main(config):
    ...
    multiply_factor = 1.0
    country_codes = ['AGO', 'BDI', 'BWA', 'COD', 'KEN', 'MDG', 'MOZ', 'MWI', 'NAM', 'TZA', 'UGA', 'ZAF', 'ZMB', 'ZWE']
    for country in country_codes:
        figures = os.path.join(figures_data_path, "country_figures", f"{country}")
        os.makedirs(figures, exist_ok=True)

        # figures = os.path.join(figures_data_path,"country_figures", f"{country}","bar_plots")
    ...
    if make_plot is True:
        ...
        for sd in scenarios_descriptions:
            ...
            for country in country_codes: 
                data_df = data_df[(data_df["iso3"]== country) & (data_df["production_tonnes"] > 0)]
    ...
    if make_plot:
        ...
        for sd in scenarios_descriptions:
            ...
            for country in country_codes:
                data_df = data_df[(data_df["iso3"].isin(country)) & (data_df["production_tonnes"] == 0)]  # metal content
                data_df["reference_mineral"] = data_df["reference_mineral"].str.title()
        ...
        for c, et, sh_c in zip(cost_columns, en_tr, short_col_name):
            ...
            for country in country_codes:
                ...
                for cs, nr in zip(constraints, figure_nrows):
                    ...
                    plt.tight_layout()
                    # Save figure
                    filename = f"{cs}_{c}_comparisons_2022_2030_2040H.png"
                    save_fig(os.path.join(figures, filename))
                    plt.close()           
    ...
    if make_plot:
        ...
        for col, col_t, m_t in zip(columns, column_titles, multiply_factors):
            ...
            for country in country_codes:
                data_df = data_df[(data_df["iso3"]==country) & (data_df["production_tonnes"] > 0)& (data_df["processing_stage"] > 0)]
                data_df["reference_mineral"] = data_df["reference_mineral"].str.title()
                ...
                for const in ['constrained', 'unconstrained']:
                    ...
                    for delta_col_name, delta_df in delta_dfs.items():
                        ...
                        # Formatting subplot
                        axs[ax_idx].set_title(f"{year}: Difference (Regional - National) \n {const.title()}", 
                    ...
                    # results_folder = os.path.join(figures_data_path, f"MR_MN/{country}")
                    os.makedirs(results_folder, exist_ok=True)
                    plt.tight_layout()
                    save_fig(os.path.join(figures, f"{col}_differences_subplot_{const}.png"))
                    plt.close()

    #########
    # Differences for most variables: unconstrained minus constrained: works
    #########
    make_plot = False
    if make_plot:
        results_file = os.path.join(output_data_path, "all_data.xlsx")
    ...
    if make_plot:
        ...
        for sd in scenarios_descriptions:
            ...
            for col, col_t, m_t in zip(columns, column_titles, multiply_factors):
                ...
                os.makedirs(results_folder, exist_ok=True)
                plt.tight_layout()
                save_fig(os.path.join(figures, f"{col}_{sc_t}_differences_subplot.png"))
                plt.close()
    ...
    if make_plot is True:
        ...
        for rf in reference_minerals:
            ...
            plt.grid()
            plt.tight_layout()
            save_fig(os.path.join(figures, f"{rf}_MN_MR_export_comparisions_side_by_side.png"))
            plt.close()
    ...
    if make_plot:
        ...
        for rf in reference_minerals:
            ...
            for country in country_codes:
                for scenario in scenarios:
                    for cs in constraints[:2]:
                        ...
                        df_c = dfs[dfs['iso3'].isin(country)]
                        # print("dfs", dfs)
                ...
                plt.tight_layout()
                save_fig(os.path.join(results, f"{rf}_unconstrained_MN_MR_production_comparisons_2030_2040.png"))
                plt.close()
            ...
            for country in country_codes:   
                ...
                plt.tight_layout()
                save_fig(os.path.join(results, f"{rf}_constrained_MN_MR_production_comparisons_2030_2040.png"))
                plt.close()
