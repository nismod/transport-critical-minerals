...
def main():
    ...
    global_df["geometry"] = global_df.geometry.centroid
    df = pd.merge(
    ...
    # for jdx, (sc_n,sc_t,df,rowpos,colpos,rowspan,colspan) in enumerate(sc_dfs):
    ...
    #                     facecolor=None
    #                     )
    #         df["markersize"] = marker_size_max*(df[sc_n]/tmax)**0.5
    #         df = df.sort_values(by=sc_n,ascending=False)
    #         df.geometry.plot(
    #             ax=ax, 
    #             color=df["color"], 
    #             edgecolor='none',
