...
def main(config,
    ...
    mineral_usage_factor_df = modify_mineral_usage_factors(
                                            scenario,
                                            future_year=year,
                                            baseline_year=baseline_year)
    # mineral_usage_factor_df.to_csv("test3.csv")
    ...
    future_proportion_df = pd.read_csv(
                            os.path.join(
                                processed_data_path,
                                "baci",
                                "baci_future_production_export_ratios.csv")
                            )[["export_country_code","reference_mineral","future_ratio"]]
    trade_proportion_df = pd.merge(
                                trade_proportion_df,
                                future_proportion_df,
                                how="left",
                                on=["export_country_code","reference_mineral"]
                                )
    del future_proportion_df
    ...
    for reference_mineral in reference_minerals:
        ...
        mines_df = mines_df.groupby(["ISO_A3"])[f"{year}_metal_content"].sum().reset_index()
        mines_df = mines_df[mines_df[f"{year}_metal_content"]>0]
    ...
    mine_exports_df.rename(
                    columns={
                            ...
                            f"{year}_metal_content":"future_metal_content_tons"
                            },
                    inplace=True)
    ...
    if scenario != "bau":
        mine_exports_df["production_to_trade_fraction"
                    ...
                    mine_exports_df["future_ratio"] == "new",
                    1, mine_exports_df["production_to_trade_fraction"]
                    )
    mine_exports_df.drop("future_ratio",axis=1,inplace=True)
    ...
    if efficient_scale == "min_threshold_metal_tons" and scenario != "bau":
        mine_exports_df["final_processing_stage"
            ...
            lambda x:x["final_refined_stage"
                        ] if x["cum_usage_factor"]*x["future_metal_content_trade_tons"
    ...
    mine_exports_df = mine_exports_df.drop_duplicates(
                            subset=[
                                    "reference_mineral",
                                    "export_country_code",
    ...
    trade_balance_df["mine_final_refined_stage"
                ...
                0)
    trade_balance_df["mine_max_refined_stage"
        ] = trade_balance_df.groupby(["reference_mineral","export_country_code"]
            )["mine_final_refined_stage"].transform("max")
    # trade_balance_df["mine_final_refined_stage"
    ...
    trade_balance_df.drop("mine_max_refined_stage",axis=1,inplace=True)
    ...
    # trade_balance_df.to_csv("test4.csv")
    trade_balance_df["future_export_tons_rem"
        ] =  np.where(
                (
    ...
    stage1_df = trade_balance_df[trade_balance_df["cum_usage_factor"] > 0]
    stage1_df["future_stage1_metal"
        ] = (1-stage1_df["cum_usage_factor"]
            )*stage1_df["future_metal_content_trade_tons"]/stage1_df["usage_factor"]
    ...
    stage1_df["future_export_tons_rem"] = stage1_df.progress_apply(
                                                lambda x:x["future_stage1_metal"
                                                ]/get_mine_conversion_factors(
                                                    x,
                                                    pr_conv_factors_df,
                                                    "initial_processing_stage",
    ...
    stage1_df = stage1_df.drop_duplicates(
                    subset=[
                            "export_country_code",
                            "reference_mineral",
                            "final_processing_stage"],
                    keep="first")
    # stage1_df.to_csv("test.csv")
    # trade_balance_df["future_export_tons_rem"
    #     ] = np.where(
    ...
    final_trade_matrix_df.to_csv(
                            os.path.join(
                                results_folder,
                                f"baci_ccg_country_trade_breakdown_{scenario.replace(' ','_')}_{year}_{percentile}_{efficient_scale}.csv"),
                            index=False)
