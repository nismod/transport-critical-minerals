...
def main(config):
    ...
    copper_mines = gpd.read_file(os.path.join(processed_data_path,
        "Minerals","copper","copper_mines.gpkg"))
    copper_processing = gpd.read_file(os.path.join(processed_data_path,
        "Minerals","copper","copper_process_sites.gpkg"))

    """Find mines attached to rail and roads
    """
    epsg_meters = 3395
    mine_processing_intersects = gpd.sjoin_nearest(copper_processing.to_crs(epsg=epsg_meters),
                            copper_mines.to_crs(epsg=epsg_meters),
                            how="left",distance_col="distance").reset_index()
    print (mine_processing_intersects)
    mine_processing_intersects = mine_processing_intersects.drop_duplicates(subset=["FeatureUID"],keep="first")
    mine_processing_intersects.to_csv("test.csv",index=False)
    # # Read files
