...
def components(edges,nodes,node_id_col,edge_id_col,from_node_col,to_node_col):
    G = networkx.Graph()
    G.add_nodes_from(
        (getattr(n, node_id_col), {"geometry": n.geometry}) for n in nodes.itertuples()
    )
    G.add_edges_from(
        (getattr(e,from_node_col), getattr(e,to_node_col),
            {edge_id_col: getattr(e,edge_id_col), "geometry": e.geometry})
        for e in edges.itertuples()
    )
    components = networkx.connected_components(G)
    for num, c in enumerate(components):
        print(f"Component {num} has {len(c)} nodes")
        edges.loc[(edges[from_node_col].isin(c) | edges[to_node_col].isin(c)), "component"] = num
